...
void ArrayBag<ItemType>::operator+=(const ArrayBag<ItemType>& a_bag){
   for (int i = 0; i < a_bag.item_count_ && item_count_ < DEFAULT_CAPACITY; i++){
...
void ArrayBag<ItemType>::operator/=(const ArrayBag<ItemType>& a_bag){
    ...
    for (int i = 0; i < item_count_; i++) {
        ItemType item = items_[i];
        if (!a_bag.contains(item) && !result.contains(item)) {
            result.add(item);
        }
    }
    for (int i = 0; i < a_bag.item_count_; i++) {
        ItemType item = a_bag.items_[i];
        if (!result.contains(item)) {
            result.add(item);
        }
    }
    
    item_count_ = result.getCurrentSize();
    for (int i = 0; i < item_count_; i++) {
        items_[i] = result.items_[i];
    }
